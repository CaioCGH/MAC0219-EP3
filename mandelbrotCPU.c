...
int main(int argc, char** argv) {
	...
	createMandelbrotImage(send_ch_buffer, chunk_size, chunk_height*(world_rank), chunk_height*(world_rank+1), width, height, a_0, b_0, a_1, b_1, maxIteration);
	int i, j;
	float *buffer;
	scaleBufferSeq(send_ch_buffer, chunk_size, 0, maxIteration);

	if(world_rank == 0)
		buffer = (float *) malloc(image_size * sizeof(float));
	...
	if(world_rank == 0){
		...
		for(i = 0; i < image_size; i++)
			printf("buffer[%d] = %f\n",i,  buffer[i]);
		#endif
		writeImage(saida, width, height, buffer, "A Mandelbrot Image");
		free(buffer);
	}
	MPI_Finalize();
}
